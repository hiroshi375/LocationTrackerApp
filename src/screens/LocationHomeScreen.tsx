...
export default function LocationHomeScreen({ navigation }: Props) {
  ...
  const [loginUserIconUrl, setLoginUserIconUrl] = useState<string | null>(null);
  ...
  useEffect(() => {
    const loadSavedHomeSettings = async () => {
      try {
        ...
        const savedSettings = JSON.parse(raw) as SavedLocationHomeSettings;
        ...
        if (savedSettings.version === LOCATION_HOME_SETTINGS_VERSION) {
          if (
            ...
            [10, 20, 50, 100].includes(savedSettings.recordDistanceMeters)
          ) {
        ...
        if (Array.isArray(savedSettings.selectedLiveShareUsers)) {
          const savedUsers = savedSettings.selectedLiveShareUsers.filter(
          ...
          try {
            ...
            if (profileResult.errors) {
              ...
              const latestProfileMap = new Map(
                (profileResult.data ?? [])
                  .filter((profile: any) => profile?.id && profile?.userId)
                  .map((profile: any) => [profile.userId, profile]),
              );

              const restoredUsers: UserProfileItem[] = savedUsers.map(
                (savedUser) => {
                  const latestProfile = latestProfileMap.get(savedUser.userId);
                  ...
                  return {
                    ...
                    displayName: latestProfile.displayName ?? null,
                    ownerValue: latestProfile.ownerValue ?? null,
                    searchText: latestProfile.searchText ?? null,
                    iconImagePath: latestProfile.iconImagePath ?? null,
                  };
                },
              );

              setSelectedLiveShareUsers(
                restoredUsers.filter((user) => !!user.ownerValue),
              );
            }
          } catch (error) {
    ...
    const saveHomeSettings = async () => {
      try {
        const settings: SavedLocationHomeSettings = {
          ...
          selectedLiveShareUsers: selectedLiveShareUsers.map((user) => ({
            id: user.id,
            userId: user.userId,
  ...
  const [pendingSessionShareOwnerValues, setPendingSessionShareOwnerValues] =
    useState<string[]>([]);
  const [pendingRecordingIntervalMs, setPendingRecordingIntervalMs] = useState<
    number | null
  >(null);
  ...
  const loadLoginUserName = useCallback(async () => {
    try {
      ...
      const name =
        profile?.displayName?.trim() || profile?.email?.trim() || "ユーザー";
  ...
  const loadLiveShareUsers = useCallback(async () => {
    try {
      ...
      if (result.errors) {
        ...
        Alert.alert("取得エラー", "共有先ユーザーを取得できませんでした。");
        return;
      }
      ...
      setSelectedLiveShareUsers((currentSelectedUsers) => {
        ...
        const latestUserMap = new Map(users.map((user) => [user.userId, user]));

        return currentSelectedUsers
          .map((selectedUser) => {
            return latestUserMap.get(selectedUser.userId) ?? selectedUser;
          })
  ...
  const handleStartRecording = async () => {
    ...
    try {
      ...
      Alert.alert("自動記録開始エラー", "自動記録を開始できませんでした。");
    } finally {
  ...
  const saveSessionName = async (name: string) => {
    ...
    const sessionName =
      trimmedName || `自動記録 ${formatDateTime(new Date().toISOString())}`;

    try {
      setSavingSessionName(true);

      const sessionLogs = await listLocationLogsBySessionId(pendingSessionId);
      ...
      if (hasErrors) {
        console.error("LocationLog session name update errors:", updateResults);
        Alert.alert("保存エラー", "アクティビティ名を保存できませんでした。");
        return;
      }
      ...
      await upsertRecordingSessionSummary(
        ...
        pendingRecordingDistanceMeters,
        {
          ...
          continuationConfirmationCount:
            matchingContinuationState?.confirmationCount ?? undefined,

          autoStoppedAt: matchingContinuationState?.autoStoppedAt ?? undefined,
  ...
  const handleDiscardSession = async () => {
    ...
    try {
      ...
      if (hasErrors) {
        console.error("LocationLog session delete errors:", deleteResults);
        Alert.alert("削除エラー", "位置情報ログを削除できませんでした。");
        return;
      }
  ...
  const handleStopRecording: () => Promise<void> =
    useCallback(async (): Promise<void> => {
      ...
      if (recordingStartedAt) {
        ...
        const stoppedSeconds = Math.floor((Date.now() - startedAtTime) / 1000);
  ...
  useEffect(() => {
    ...
    const finalizeAutoStoppedSession = async () => {
      ...
      try {
        ...
        const matchingContinuationState =
          continuationState?.recordingSessionId === autoStoppedSessionId
            ? continuationState
            : null;
        ...
        console.error("Finalize auto-stopped RecordingSession error:", error);
  ...
  const canOpenRecordingMap = isRecording && Boolean(activeRecordingSessionId);
    ...
    try {
      ...
      do {
        ...
        if (result.errors) {
          ...
          Alert.alert("取得エラー", "共有中の現在地を取得できませんでした。");
          return;
        }
      ...
      const sharedLiveLocations: LiveLocationItem[] = allData
        ...
        .sort((a, b) => {
          const aTime = new Date(a.updatedAt ?? a.recordedAt ?? 0).getTime();
          const bTime = new Date(b.updatedAt ?? b.recordedAt ?? 0).getTime();
      ...
      const sharedLiveIsRecording =
        latest.isRecording === true && Boolean(latest.recordingSessionId);
      ...
      Alert.alert("取得エラー", "共有中の現在地を開く処理に失敗しました。");
    } finally {
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              const result = await backfillRecordingSessionsFromLocationLogs(
                (progress) => {
              ...
              console.error("RecordingSession backfill error:", error);
    ...
    >
      ...
      >
        <View style={styles.userInfoBox}>
          ...
          {loginUserIconUrl ? (
            <Image source={{ uri: loginUserIconUrl }} style={styles.userIcon} />
          ) : (
        ...
        <View style={styles.liveShareBox}>
          <Text style={styles.liveShareTitle}>リアルタイム共有先</Text>
          ...
          {selectedLiveShareUsers.length > 0 && (
            <Text style={styles.liveShareSelectedUserName}>共有先ユーザー</Text>
          )}

          {selectedLiveShareUsers.length > 0 && (
            <View style={styles.liveShareUserIconRow}>
              {selectedLiveShareUsers.map((user) => {
                const userName = user.displayName || user.email || "名前未設定";

                const iconUrl = liveShareUserIconUrls[user.id];

                return (
                  <View key={user.id} style={styles.liveShareUserIconItem}>
                    {iconUrl ? (
                      <Image
                        source={{ uri: iconUrl }}
                      ...
                      <View style={styles.liveShareUserIconPlaceholder}>
                        <Text style={styles.liveShareUserIconPlaceholderText}>
                          {userName.slice(0, 1)}
                        </Text>
                      </View>
                    )}
        ...
        <View style={styles.autoRecordBox}>
            ...
            <View style={styles.recordingStatusArea}>
              {isRecording ? (
                ...
                <View style={styles.stoppedBadge}>
                  ...
                  <Text style={styles.stoppedBadgeText}>停止中</Text>
                </View>
              )}
            </View>
          </View>
          ...
          {isRecording && (
            <View
              style={[
                styles.backgroundTaskStatusContainer,
                backgroundTaskStatus === "error" &&
                  styles.backgroundTaskStatusErrorContainer,
              ]}
            >
              <Text
                style={[
                  styles.backgroundTaskStatusText,
                  backgroundTaskStatus === "error" &&
                    styles.backgroundTaskStatusErrorText,
                ]}
              >
                バックグラウンド記録：
                {formatBackgroundTaskStatus(backgroundTaskStatus)}
              </Text>

              {backgroundTaskErrorMessage ? (
                <Text style={styles.backgroundTaskErrorMessage}>
                  {backgroundTaskErrorMessage}
                </Text>
              ) : null}
            </View>
          ...
          <View style={styles.settingBlock}>
            ...
            <View style={styles.optionRow}>
              {RECORD_INTERVAL_OPTIONS.map((option) => {
                const selected = recordIntervalMs === option.value;
                ...
                return (
                  <Pressable
                    ...
                    style={[
                      ...
                      isRecording && styles.optionButtonDisabled,
                    ]}
                    onPress={() => setRecordIntervalMs(option.value)}
                  >
                    <Text
                      style={[
          ...
          <View style={styles.settingBlock}>
            <Text style={styles.settingTitle}>記録する移動距離</Text>

            <View style={styles.optionRow}>
              {DISTANCE_OPTIONS.map((option) => {
                const selected = recordDistanceMeters === option.value;
                ...
                return (
                  <Pressable
                    ...
                    onPress={() => setRecordDistanceMeters(option.value)}
                  >
                    <Text
                      style={[
                        styles.optionButtonText,
          ...
          {isRecording ? (
            <Pressable
              style={({ pressed }) => [
                ...
                pressed && !stoppingRecording && styles.buttonPressed,
            ...
            >
              <Text style={styles.autoRecordButtonText}>
                {stoppingRecording ? "停止処理中..." : "自動記録停止"}
              </Text>
            </Pressable>
          ) : (
            <Pressable
              ...
              disabled={!hasLoadedSavedHomeSettings || startingRecording}
            >
              <Text style={styles.autoRecordButtonText}>
                {!hasLoadedSavedHomeSettings
                  ? "設定を読み込み中..."
          ...
          <View style={styles.buttonSpace}>
            ...
            {backfillingSessions && backfillProgress && (
              <View style={styles.backfillProgressContainer}>
                ...
                {backfillProgress.phase === "processingSessions" &&
                  backfillProgress.totalSessionCount > 0 && (
                    <>
                      <View style={styles.backfillProgressTrack}>
                        <View
                          style={[
                      ...
                      <Text style={styles.backfillProgressDetail}>
                        成功: {backfillProgress.createdOrUpdatedCount}
                        件　失敗: {backfillProgress.failedCount}件
                      </Text>
                    </>
                  )}
              </View>
        ...
        >
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent}>
              <Text style={styles.modalTitle}>セッション名を入力</Text>
              ...
              <View style={styles.modalButtonRow}>
                <Pressable
                  style={[
                    ...
                    savingSessionName && styles.appButtonDisabled,
                  ]}
                ...
                <Pressable
                  ...
                  disabled={savingSessionName}
                  onPress={() => saveSessionName(sessionNameInput)}
                >
                  <Text style={styles.modalPrimaryButtonText}>
                    {savingSessionName ? "保存中..." : "保存"}
                  </Text>
                </Pressable>
              </View>
            </View>
        ...
        >
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent}>
              <Text style={styles.modalTitle}>リアルタイム共有先を選択</Text>
              ...
              <ScrollView
                ...
                contentContainerStyle={styles.liveShareUserListContent}
                keyboardShouldPersistTaps="handled"
              >
                {loadingLiveShareUsers ? (
                  ...
                  <Text style={styles.liveShareEmptyText}>
                    共有先ユーザーが見つかりません。
                    {"\n"}
                    UserProfile に他のユーザーが存在するか確認してください。
                  </Text>
                ) : (
                  filteredLiveShareUsers.map((user) => {
                    const selected = draftLiveShareUsers.some(
                    ...
                    return (
                      <Pressable
                        ...
                        style={[
                          ...
                          selected && styles.liveShareUserItemSelected,
                        ]}
                        onPress={() => toggleLiveShareUser(user)}
                      >
                        <Text style={styles.liveShareUserName}>
                          {user.displayName || "名前未設定"}
                        </Text>

                        <Text style={styles.liveShareUserEmail}>
                          {user.email || "メールなし"}
                        </Text>
                      </Pressable>
                    );
...
function formatBackgroundTaskStatus(
  status:
    | "idle"
    | "checking"
    | "registered"
...
const styles = StyleSheet.create({
  ...
  backgroundTaskStatusErrorContainer: {
